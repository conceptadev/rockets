...
> Canonical reference app for `@conceptadev/rockets` micro apps — JWT auth,
> zod-first resources, owner scoping, and `defineTypeOrmRepository`.

Monorepo dev: `@conceptadev/*` resolves via `workspace:^` to local
`packages/*`. External apps install from npm (`@conceptadev/rockets@alpha`).
...
`@conceptadev/rockets`. It exists to:
...
1. `src/resources/<thing>/<thing>.schema.ts` — zod schema + `f.*` field helpers (`@conceptadev/rockets-core/zod`).
