...
test for in-development SDK changes. The published line is
`0.1.0-alpha.2` under `@alpha`; the older `0.0.1-dev.0` on `dev` is not
supported.
...
```text
Web (Firebase client SDK)               API (@concepta/rockets)
───────────────────────────             ───────────────────────────
...
The `build` step compiles every local `@concepta/*` package — required
...
| Code-review reports | Firestore via `@concepta/rockets-repository-firestore` | `FIREBASE_FIRESTORE_REPORTS_COLLECTION` (default `code_review_reports`) |
...
| `FIREBASE_SERVICE_ACCOUNT_PATH` / `GOOGLE_APPLICATION_CREDENTIALS` | Path to a Firebase service-account JSON (read by `@concepta/rockets-repository-firestore`). |
...
|---|---|
| `@concepta/rockets@0.0.1-dev.0` from npm | `workspace:^` → local `packages/*` (integration test for in-development SDK) |
| Built-in `@concepta/rockets-auth` | Firebase via `defineFirebaseAuth()` |
