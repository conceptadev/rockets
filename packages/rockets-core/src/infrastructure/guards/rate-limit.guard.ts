...
export class RateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const merged: Record<string, RateLimitOptions> = {};
    for (const policy of [
    ...
    for (const [name, options] of dimensions) {
      ...
      if (
        verdict === undefined ||
        (!result.allowed && verdict.result.allowed) ||
        (result.allowed === verdict.result.allowed &&
          result.remaining < verdict.result.remaining)
      ) {
        verdict = { result, name };
      }
    }
    ...
    response.setHeader?.('X-RateLimit-Limit', String(verdict.result.limit));
    response.setHeader?.(
    ...
    response.setHeader?.(
      'X-RateLimit-Reset',
      String(Math.ceil(verdict.result.resetAt / 1000)),
    );
