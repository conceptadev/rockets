...
export class ModuleAFixture {
  constructor(private moduleB: ModuleBFixture, private moduleC: ModuleCFixture) {
    initModuleATranslation();
    initModuleBTranslation();
    initModuleCTranslation();
  }
  translationModuleA() {
     return I18n.translate({ namespace: ModuleAFixture.name, key: 'hello', language: 'en' });
  }
  ...
  translationC(key:string = 'hello', language: string = 'en') {
    return I18n.translate({ namespace: ModuleCFixture.name, key: key, language: language });
  }
}