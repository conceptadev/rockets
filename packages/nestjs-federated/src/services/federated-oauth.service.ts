...
import { FEDERATED_MODULE_USER_LOOKUP_SERVICE_TOKEN, FEDERATED_MODULE_USER_MUTATE_SERVICE_TOKEN } from '../federated.constants';
...
export class FederatedOAuthService implements FederatedOAuthServiceInterface
{
  ...
  async sign(provider: string, email: string,  subject: string): Promise<FederatedCredentialsInterface> {
    const federated = await this.federatedService.exists(provider, subject);
    
    // if there is no federated user, create one
    if (!federated) {
      
      const newUser = await this.createUserWithFederated(provider, email, subject);
      return newUser;
   ...
   * @return 
   */
  private async createUserWithFederated(provider: string, email: string, subject: string): Promise<FederatedCredentialsInterface> {
   ...
   * @return 
   */
  private async  createUser (email: string, username: string): Promise<FederatedCredentialsInterface> {
   ...
   * @return 
   */
  private async  createFederated (provider: string, subject: string, userId:string): Promise<FederatedEntityInterface> {
