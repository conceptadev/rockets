...
describe('loggerSentryConfig', () => {
  ...
  it('loggerSentryConfig.logLevelMap', async () => {
    ...
    expect(loggerSentryConfig().logLevelMap('error')).toBe(SentryLogSeverity.Error)
    expect(loggerSentryConfig().logLevelMap('debug')).toBe(SentryLogSeverity.Debug)
    expect(loggerSentryConfig().logLevelMap('log')).toBe(SentryLogSeverity.Log)
    expect(loggerSentryConfig().logLevelMap('warn')).toBe(SentryLogSeverity.Warning)
    expect(loggerSentryConfig().logLevelMap('verbose')).toBe(SentryLogSeverity.Info)
